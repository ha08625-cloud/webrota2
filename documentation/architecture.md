...
1. **Backend test suite** — `uv sync --extra dev`, then the full pytest suite (engine + API) on in-memory SQLite. API tests run `TestClient` with `get_db` dependency-overridden to a fresh SQLite engine per test. Client fixtures: `client` also overrides `get_current_user` with a manager-tier stub user, so most tests don't need to authenticate to exercise a route; `client_no_auth` leaves `get_current_user` un-overridden to exercise the real login/session path (auth plan, Task 4); `client_at_tier(level)` (plus `viewer_client` / `admin_client` / `manager_client`) stubs a chosen access level. All of these write to the one `app.dependency_overrides` dict on the one shared `app`, so exactly one client per test — `client_at_tier` raises on a second call rather than silently retargeting the first. See `backend/tests/test_api/conftest.py`.
