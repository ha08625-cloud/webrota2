...
[UNRESOLVED: this section now covers 001–005. Migrations **006, 007, and 008 remain undocumented here**. 006 (`generated_rota_committed_at.py`) adds `GeneratedRota.committed_at`, supporting the rollback-of-commits work already live in the frontend (`RotaDetailPage.tsx`'s rollback UI, `POST /rota/{id}/rollback-commit`) but never written up here or in "Rota lifecycle" / "Router surface" below. 007 (`rota_generation_log.py`) adds the `rota_generation_log` table — already described under "Tables" above (the `rota_generation_log` entry cites migration 007 by name) but has no corresponding paragraph in this Migrations section, an inconsistency worth fixing in the same pass. **008 (`generated_rota_archived_at.py`) is a separate, apparently-complete feature**: `GeneratedRota.archived_at`, an `/archive` and `/unarchive` router surface, schema fields on both `RotaSummary` and the detail schema, and dedicated tests all exist already (code comments refer to this as "M6"), none of it reflected anywhere in this file. Needs its own documentation pass, distinct from clinic-priority.]
