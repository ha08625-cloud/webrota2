...
- **Phase 9C — trainee supervision.** Assigns a Partner/Salaried supervisor to every session with at least one trainee requiring supervision (SR-room occupant first if eligible, otherwise lowest weighted `SUPERVISION` score). Diverges from the original GAS design in three confirmed places: no duty-helper fallback pool (a duty helper's `role=CLINIC` already excludes them from the single pool); the SR-priority doctor's counter is incremented like any other selection; the trainee count itself is never persisted, only `is_supervising: bool`, since `is_on_leave` is derived at read time and a stored count would go stale. Exposes `count_supervisable_trainees`/`is_eligible_supervisor` as shared predicates so Phase 12 Check 4 cannot drift from the assignment rule. See phase-pipeline.md for the full specification.
...
- **App.tsx** — routing and layout only, no providers: `BrowserRouter` (React Router v7, declarative mode — TanStack Query owns all server state, so no router loaders/actions), a fixed left nav, and the nine routes (Rota list/detail, Master Rota, Clinic Types, Doctors, Leave, Duty, Closures, Counters).
...
|---|---|---|---|
| Auth (real) | Python only | Deferred post-project | Replaces the shared-token shim with users/login. Enforcement lives entirely in `get_current_user()` at the router layer, so auth requires zero changes to route handlers or business logic. Done when the app rejects unauthenticated requests and the 2–3 admin users can log in (simple username/password first) |
...
| Auth | Shared-token shim: `X-API-Token` header vs `API_TOKEN` env var |
