...
Phase 9C    -> Assign trainee supervision, and seat each chosen supervisor in
               the SR room reserved for that session
...
- **Phase 9C** was ported with three deliberate divergences from the original GAS design, and now runs *before* Phases 7-9A rather than after 9B - see the Phase 9C section below.
...
(`phase0.py` also emits several duty-related errors that postdate this list - see the Phase 0 bullet in `architecture-clinical.md`.)

**SR warnings (severity="warning" - they do not abort the run):** two checks flag stored data that would defeat Phase 9C's SR reservation, since Phase 0 is the only place that can name the *cause* before the run rather than the symptom after it:

- `pre_assigned_sr_room` - a `PRE_ASSIGNED` template row naming an SR room. Phase 2 claims such a room before any reservation applies, so the row wins outright and Phase 9C finds no free SR room in that week's sessions. One finding per template row, not per generation week: the fix is to edit that one row. A warning rather than an error because the row is not necessarily wrong - the sessions it covers may have no trainees, and Phase 0 has no grid to tell, running as it does before Phase 2 builds one.
- `clinic_sr_room_eligibility` - an enabled clinic type with a `ClinicTypeRoomEligibility` row naming an SR room. The clinic-type API rejects these now (see the Phase 5 section), but a row predating that change, or written straight against the database, still resolves in Phase 5. Deliberately a warning rather than a silent filter in `load_context()`: the engine quietly disagreeing with stored data is exactly the failure mode the decision log exists to prevent. `context.clinic_types` is enabled-only, so a disabled type carrying a stale row is correctly silent.
...
**Room resolution, per duty row, first pass:** already in a D room -> done. Otherwise try the doctor's preferred D room: free -> take it; occupied by a protected doctor (a nurse, who is inert; a Partner or AHP, by seniority; or anyone already holding a role) -> fall through to the sweep; occupied by anyone else -> evict and relocate them (Salaried via the shared preference-then-C/W/SR search; Trainee/Locum via a D-room-only search, Design Decision 8), then take the room. Eviction is unconditional - the duty doctor takes the room even if the evictee cannot be rehoused (Design Decision 10), and the evictee's `ROOM_MOVE` counter increments either way.

**Reserved SR rooms.** The evictee relocation search skips the SR room reserved for supervision in that session. The reservation covers only sessions with at least one supervisable trainee - elsewhere SR is an ordinary room in the pool, so it is not left standing empty. `phase9c.reserved_sr_room_ids()` is the single definition of the rule; `generate.py` computes it once off the Phase 2 grid and passes the same mapping to Phases 4 and 5, which are the only phases that place a doctor *before* Phase 9C books the room. The check itself lives inside `room_relocation.find_relocation_room()`, shared with Phase 7-9A Pass 2 - a no-op there, since by then the supervisor already occupies the room and `grid.is_room_free()` excludes it anyway.
...
- Room occupied by a nurse -> protected, leave both doctors where they are. A nurse is inert: no phase moves one, and this bump is the one place in Phase 4 that would otherwise have, since it is not gated by `_protection`. Logged with its own narrator reason rather than a bare skip, so the log says why the consolidation did not happen.
- Room occupied by another doctor already on `DUTY_PRIMARY`/`DUTY_SECONDARY` that session -> protected, leave both doctors where they are. Apart from the nurse case above, this protection rule is narrower than the preferred-room step's `_protection` - Partners, AHPs and clinic-role holders are *not* protected here, and may be bumped.
...
**Eligibility** for a given clinic/day/period: the doctor must be in the clinic's `doctor_eligibilities`, active, not a nurse, have a session slot at that time that is not on leave, not WFH, not `NO_SURGERY`/`ADMIN_TIME`, and not already holding a role (duty or an earlier-tier clinic). The nurse test is a property of the doctor rather than of the slot, so it sits with the active check and ahead of any slot lookup, and it records an exclusion reason in the decision log like every other filter here. The clinic-type API rejects a nurse eligibility outright, so it only ever fires on rows stored before that landed.

**Selection** among eligible doctors: lowest `doctor_priority` first, then lowest weighted counter score, then alphabetical by code. The weighted score is `(raw_count + adjustment) / sessions_per_week`, so part-time doctors accumulate assignments proportionally to their working week, and a doctor whose counter has been adjusted is not treated as under-loaded when the raw count misrepresents their fair share — a mid-year joiner, or a return from exceptional absence (see "Counters and snapshots" in architecture-clinical.md; the adjustment is admin-set, defaults to zero, and the engine never writes it). The same formula is used everywhere a weighted counter is compared (clinic selection here, `ROOM_MOVE` displacement candidates in Phase 4's fallback sweep and Phase 7-9A, and `SUPERVISION` selection in Phase 9C).

**Room resolution** (only if `room_required`): already-eligible room -> free eligible room -> displace an occupant of an eligible room who isn't a nurse (never displaceable, whatever the clinic's priority - the check lives in `_log_phase5.displaceability()`, which returns the reason the log prints alongside the verdict) or on leave/duty/protected by a higher-priority clinic, moving them to their best free preferred room (excluding D-type rooms if the room being freed is itself a D room) -> warning and leave the doctor in their current room if none of that works. This uses only the displaced doctor's own preference list - no fallback to any-free-room-of-a-type, unlike the fuller displacement algorithm used in Phase 7-9A (see below). Room resolution success or failure does not affect whether the clinic counter is incremented - the counter tracks the clinic assignment, not the room outcome.

**Reserved SR rooms.** As in Phase 4, the displaced-occupant search skips the SR room reserved for supervision in that session (`phase9c.reserved_sr_room_ids()`, the single definition of the rule - see the Phase 4 section above). The clinic's *own* room search needs no such check: SR is no longer a selectable clinic room at all (rejected by the clinic-type API, filtered out of the room picker), so `eligible_room_ids` cannot name one. Stored data predating that change still can, which is why Phase 0 warns on a clinic type eligible for an SR room rather than the engine silently filtering it - see Phase 0 above.
...
**Depends on:** Phase 9C (which in turn depends on Phase 5). Rooms are resolved *after* supervision is assigned, not before - see the Phase 9C section for why.  
...
Throughout this section **"Trainee/AHP" is shorthand for the D-room demand set**, `phase7_9a._D_ROOM_TYPES`: Trainee, AHP and Locum. Locum behaves as Trainee-minus-supervision; the constant, not this prose, is what the code branches on. **Nurse is deliberately absent** — a nurse is inert to the engine and generates no D-room demand, so no nurse is ever a Pass 1 or Pass 2 subject, and `_DISPLACEABLE_TYPES` excludes them from being a victim or a Pass 3 subject too. A nurse left holding an unresolved `REQUIRES_ROOM` slot is reported by Phase 12, not roomed here. See "`DoctorType`" in architecture-clinical.md for the full inertness rule.
...
**Pass 3 (Partner/Salaried fallback, was GAS Phase 9A):** no displacement. Walk the doctor's own preference list first and take the first free room. If nothing on their own list is free, force the doctor into the first free room by type priority D > C > W > SR (`phase7_9a._PASS3_FALLBACK_TYPE_ORDER`, rooms ordered by code within a type) — Pass 3's own fallback pool, distinct from the Pass 1/2 displaced-doctor pool (C/W/SR, D excluded) below. D rooms are deliberately included here: Pass 3 runs last, after Passes 1 and 2 have already settled all Trainee/AHP D-room demand, so any D room still free at this point is genuine surplus. Only if the preference list and the full fallback sequence are both exhausted does the slot remain unresolved and a warning fire.
...
**Supervisors are immobile.** Passes 1 and 2 will not displace a slot with `is_supervising=True`: once Phase 9C has seated a supervisor, that slot's room is fixed for the rest of the run. Pass 1 moves both of a day's sessions, so it rejects the doctor if *either* the AM or the PM slot is supervising; Pass 2's test is per-period. These guards are a backstop rather than the usual path - Passes 1 and 2 displace D-room occupants, and a supervisor is in SR - but they are what keeps the invariant if the SR booking ever fails (a `PRE_ASSIGNED` SR row, see Phase 9C) and Pass 3 seats a supervisor in a D room.

Pass 3 itself is unchanged, but now runs after Phase 9C, which matters in two ways. A supervisor arrives at Pass 3 already holding SR, so Pass 3 never has to room one. And in a session with supervisable trainees the SR room is occupied by the time Pass 3 walks its `D > C > W > SR` fallback, so `grid.is_room_free()` excludes it with no reservation check needed - the same is true of Pass 1's fixed C/W/SR pool and Pass 2's relocation search. The cost is that one fewer room is available to everyone else in a supervised session, which can turn Pass 3's "no free room anywhere" warning from theoretical into real in a tight week. Accepted: seating the supervisor is the higher priority, and the warning already exists to report it.
...
**Supervising PM slots are excluded.** A pair is also skipped if either doctor's **PM** slot has `is_supervising=True` - Phase 9C seated that doctor in SR for the afternoon and 9B must not move them out. The guard is deliberately PM-only, matching the fact that 9B only ever writes PM: an *AM* supervisor is unaffected, and guarding on AM as well would abandon resolutions that are harmless, since DEFAULT sets PM to the AM room, which for an AM supervisor means moving them *into* SR for PM rather than out of it.

Because 9C now runs before 7-9A rather than after 9B, 9B is also no longer at risk of having its own guarantee undone: a supervision room move can no longer reintroduce the mid-day swap 9B had just removed. Note this is the reorder's benefit, not a new repair duty - the exclusion above means 9B never resolves a supervising pair in the first place.
...
**Writes:** `is_supervising=True` on the selected slot; increments the selected doctor's `SUPERVISION` system counter; and books the selected supervisor into an SR room, freeing whatever room they were holding - including a `PRE_ASSIGNED` one. Emits a `supervision_unassignable` warning when no selectable supervisor exists for a session.  
**Depends on:** Phase 5 (duty and clinic roles must be applied, so `role is None` correctly excludes them). Rooms are deliberately **not** final coming in: this phase runs *before* Phases 7-9A, so nearly every candidate is still roomless, and the room this phase writes is the supervisor's SR booking rather than an adjustment to someone else's.  
**Python implementation:** `run_phase9c(context, grid, counters, log)` in `phase9c.py`. Exposes three shared, module-level public functions - `count_supervisable_trainees`, `is_eligible_supervisor` and `reserved_sr_room_ids`. The first two are reused verbatim by Phase 12 Check 4, so the assignment rule and the validation rule cannot drift apart; the third is the single definition of the SR reservation, consulted by Phases 4 and 5.

**Why it runs here.** Three reasons, in order of weight. Booking a room up front beats swapping after the fact: before 7-9A the reserved SR room is free in every session that needs it, so the supervisor simply takes it - no displaced doctor, no cascading room churn. The reorder is also what makes the reservation cheap: of the seven code paths that could put a non-supervisor in SR, four live in Phases 7-9A, which now run afterwards and are excluded for free by `grid.is_room_free()`; only Phase 4's and Phase 5's relocation searches need an explicit check, and Phase 5's clinic search is closed by SR not being a selectable clinic room at all. And running before 9B removes an exposure that existed when 9C ran last - a single-period supervision room move could reintroduce exactly the mid-day swap 9B had just eliminated.
...
2. **No SR-priority fast path.** GAS (and an earlier iteration of this port) auto-assigned the SR-room occupant ahead of the pool comparison, whether or not they were the fairest choice by counter. That fast path has been removed: selection is always by weighted `SUPERVISION` score across the whole pool. It is now doubly moot - at this point in the pipeline the pool has no room criterion at all, so there is nothing for such a fast path to key on, and the SR occupant is the supervisor by construction rather than by shortcut. (Kept as a numbered divergence rather than dropped and renumbered to two: it still names a real difference from the GAS algorithm a reader may be porting against, and `phase9c.py`'s own module docstring numbers it the same way - renumbering here would put the two lists out of step for no gain.)
...
**Trainee counting rule** (`count_supervisable_trainees`): a trainee counts toward "needs supervision" iff their slot exists, `doctor_type == TRAINEE`, not on leave, not WFH, and `template_type` is not `NO_SURGERY`/`ADMIN_TIME`. **No room criterion:** a trainee holding a C or W room still counts - this matches GAS and is the intended model, not an oversight.

**Supervisor eligibility - two predicates.** Selection and validation happen at opposite ends of the pipeline, where the same room test means different things, so the rule is split:

- `is_selectable_supervisor(context, grid, slot)` - what this phase builds its pool from. Partner or Salaried; `role is None` (excludes duty doctors, clinics, and duty helpers); not on leave; not WFH; `template_type` not in (`NO_SURGERY`, `ADMIN_TIME`). **No room criterion at all** - not "no room, or a D/SR room", none. At this point a role-free Partner/Salaried doctor holds a room only if a `PRE_ASSIGNED` template row gave them one (Phases 4 and 5 only ever *relocate* doctors who already hold one), so a room test would empty the pool in nearly every session. It is safe to drop because the chosen supervisor is moved into SR regardless of what they were holding.
- `is_eligible_supervisor(context, grid, slot)` - the same rule *plus* the D-or-SR room test, used by Phase 12 Checks 4a/4b, which run at the end of the pipeline when rooms really are final.

`is_eligible_supervisor` is defined as `is_selectable_supervisor(...) and <room is D or SR>` rather than restating it, so the two cannot drift. Both live in `phase9c.py` as module-level public functions and are not to be inlined elsewhere.

**Per session, in order:**

1. Count supervisable trainees. Zero -> skip the session entirely.
2. Build the pool of every `is_selectable_supervisor` slot in the session - no room criterion, no SR-first shortcut.
3. Empty pool -> `supervision_unassignable` warning, no assignment (Phase 12 Check 4 also independently reports the session).
4. Otherwise select by lowest weighted `SUPERVISION` score (`(raw_count + adjustment) / sessions_per_week`; `spw=0` scores infinity, never selected), scaled by the doctor's `supervision_preference` multiplier, alphabetical tiebreak by doctor code - the same selection pattern as Phase 5. Sets `is_supervising=True` and increments the selected doctor's `SUPERVISION` counter.
5. **SR booking:** seat the supervisor in the first free SR room (rooms ordered by `code` for determinism - the schema does not constrain SR to exactly one room even though the seed currently has one), freeing whatever room they held. Under the reservation this always succeeds, so in practice the supervisor always ends up in SR. Two no-ops: a supervisor already sitting in an SR room is already where this would put them; and if no SR room is free at all, the supervisor is left roomless for Pass 3 to place normally and Phase 12 flags the outcome. The booking is a pure room write - it touches neither `is_supervising` nor any counter, `ROOM_MOVE` included, since the doctor is being seated for the job they were just given rather than displaced for someone else's.

   The move is unconditional even when the room being vacated is a `PRE_ASSIGNED` one. That is a deliberate override of a template pin, and the only one Phase 9C makes - without it, a role-free Partner/Salaried doctor pinned to a C or W room would be selectable under `is_selectable_supervisor`, keep that room, and trip Phase 12's `supervision_on_incompatible_slot` on every run. The generation-log entry for the booking says so in its rationale.

**The one hole in "the supervisor always ends up in SR":** a `PRE_ASSIGNED` template row naming an SR room. Phase 2 claims that room before any reservation can apply, so it wins, step 5 finds nothing free, and the supervisor is roomed by Pass 3 like anyone else. Phase 0's `pre_assigned_sr_room` warning makes the conflict visible up front and Phase 12 reports the consequence - a data problem with a named cause, not a silent failure. Teaching Phase 2 about supervision was rejected as the cure: Phase 2's job is to lay down the template as written.

**Accepted consequence - supervisors change room at lunchtime.** A supervisor sits in SR for the period they supervise and takes an ordinary room from Pass 3 for the day's other period, so a doctor supervising only one session of a day now moves room mid-day where previously they usually would not. This is the one change a reader of the finished rota will actually notice. It is accepted rather than mitigated: the practice's doctors prefer not to supervise for a whole day, so a single supervised session is the common case by design, and the alternative - holding SR for them across both periods - would waste the room for the unsupervised half.

**Reserved SR rooms** (`reserved_sr_room_ids`): for each `(week, day, period)` with at least one supervisable trainee, the first SR room by code is held back from the room searches in Phases 4 and 5 - the only phases that place doctors before this one. Sessions with no supervisable trainees get no reservation, so SR stays in the ordinary pools there rather than standing empty, and a doctor who prefers SR still gets it. The map is computed once, off the Phase 2 grid, which carries one caveat: Phase 4 clears `is_wfh` when it applies a duty role, so a trainee WFH slot that Phase 4 turns into a working one is not counted and its session gets no reservation. Phase 9C still books an SR room there if one happens to be free.
...
**Depends on:** Phase 9B, and Phase 9C for Check 4. Phase 9C is no longer the phase immediately before this one - it now runs before Phases 7-9A - but it still runs earlier in the pipeline, which is all Check 4 requires. Every other check needs only the room resolution that 7-9A and 9B complete.  
...
| 4a. Supervision missing | Every session with `count_supervisable_trainees >= 1` has at least one slot that is both `is_supervising=True` and currently `is_eligible_supervisor` (the room-aware predicate - see Phase 9C) - a flag on an ineligible slot does not satisfy this | Implemented (`supervision_missing`) |
