...
- Room occupied by another doctor already on `DUTY_PRIMARY`/`DUTY_SECONDARY` that session -> protected, leave both doctors where they are. This protection rule is narrower than the preferred-room step's `_is_protected_occupant` above - Partner/AHP/Nurse and clinic-role holders are *not* protected here, and may be bumped.
...
Throughout this section **"Trainee/AHP" is shorthand for the D-room demand set**, `phase7_9a._D_ROOM_TYPES`: Trainee, AHP, Nurse and Locum. Nurse is rule-identical to AHP here and Locum to Trainee-minus-supervision; the constant, not this prose, is what the code branches on.
