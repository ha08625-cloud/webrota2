...
**Pass 1 (full-day Trainee/AHP, was GAS Phase 7):** for a Trainee/AHP needing the same D room for both AM and PM of a day: prefer a room already free in both sessions; failing that, find a full-day Partner/Salaried D-room occupant who isn't on leave/duty/clinic-assigned, tie-broken by lowest weighted `ROOM_MOVE` score then alphabetically, and displace them.
...
**Pass 3 (Partner/Salaried fallback, was GAS Phase 9A):** no displacement. Walk the doctor's own preference list and take the first free room. If nothing in their own list is free, warn and leave the slot unresolved - this pass deliberately does not fall back to any other free room, unlike Pass 1/2's displacement logic below.
