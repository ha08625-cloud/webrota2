...
path for a database with no manager in it -- users.access_level defaults
to "nurse", so a row inserted without one is not a manager, and defaulting
this to anything lower would defeat the point.
