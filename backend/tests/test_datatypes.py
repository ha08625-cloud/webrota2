...
class TestCounterState:
    ...
    def test_adjustment_is_added_to_the_numerator(self):
        # The joiner credit: a doctor with raw 0 and a 3.2-session adjustment
        # scores exactly as if they had already done 3.2 sessions.
        cs = CounterState(clinic_adjustment={(1, 1): 3.2})
    ...
    def test_system_adjustment_for_is_added_and_then_scaled(self):
        # The multiplier applies to the whole (raw + adjustment) / spw score,
        # not to the raw count alone.
        cs = CounterState(
            system={(1, SystemCounterType.SUPERVISION): 2},
            system_adjustment={(1, SystemCounterType.SUPERVISION): 2.0},
        )
    ...
    def test_adjustment_sorts_a_doctor_behind_an_identical_peer(self):
    ...
    def test_negative_adjustment_is_allowed(self):
        cs = CounterState(clinic={(1, 1): 4}, clinic_adjustment={(1, 1): -2.0})
        assert cs.weighted_clinic_score(1, 1, spw=4.0) == pytest.approx(0.5)

    def test_spw_zero_returns_inf_even_with_a_adjustment(self):
        cs = CounterState(
            clinic_adjustment={(1, 1): 5.0},
            system_adjustment={(1, SystemCounterType.ROOM_MOVE): 5.0},
        )
        assert cs.weighted_clinic_score(1, 1, spw=0) == math.inf
        assert cs.weighted_system_score(1, SystemCounterType.ROOM_MOVE, spw=0) == math.inf

    def test_increment_never_touches_the_adjustment(self):
        # The snapshot contract: the engine moves raw counts only, so a
        # adjustment can never be captured, restored or corrupted by a draft.
        cs = CounterState(
            clinic_adjustment={(1, 1): 3.0},
            system_adjustment={(1, SystemCounterType.ROOM_MOVE): 3.0},
        )
        cs.increment_clinic(1, 1)
        cs.increment_system(1, SystemCounterType.ROOM_MOVE)
        assert cs.clinic[(1, 1)] == 1
        assert cs.system[(1, SystemCounterType.ROOM_MOVE)] == 1
        assert cs.clinic_adjustment == {(1, 1): 3.0}
        assert cs.system_adjustment == {(1, SystemCounterType.ROOM_MOVE): 3.0}

    def test_adjustment_alone_does_not_make_a_clinic_key_new(self):
        # A adjustment is loaded from an existing DB row, so its key must not
        # be reported as needing an INSERT.
        cs = CounterState(clinic={(1, 1): 0}, clinic_adjustment={(1, 1): 2.0})
        cs.increment_clinic(1, 1)
        assert cs.is_new_clinic_key(1, 1) is False

    def test_missing_adjustment_key_reads_as_zero(self):
        cs = CounterState()
        assert cs.clinic_adjustment_for(1, 1) == 0.0
        assert cs.system_adjustment_for(1, SystemCounterType.ROOM_MOVE) == 0.0
