...
this schema (see 001_initial_schema.py's docstring); the User -> UserSession
cascade is ORM-level (relationship(cascade="all, delete-orphan")), the same
pattern GeneratedRota already uses for
RotaSession/RotaClosure/generation_log.
...
access_level is the legacy permission tier and nothing authorizes off it
any more -- but it stays, because the audit log snapshots it, "Manager" is
...
functional gain. It defaults to NURSE -- the lowest tier -- as both the
Python-side default and the server_default: an accidental viewer is
recoverable, an accidental manager is a silent security hole. The API never
relies on that default (UserIn requires access_level), so the
server_default only ever applies to rows inserted by direct SQL, e.g. by a
