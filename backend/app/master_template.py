...
def load_week_one_template(db: Session) -> dict[tuple[int, Day, Period], MasterSessionType]:
    ...
    mapping a date onto the 4-week cycle needs a `start_week`, and the only
    source of one is `RotaConfig.template_start_week` -- a per-run value with no
    calendar anchor. Partner, salaried, and locum doctors work the same sessions
    every week, so week 1 is the right answer for leave planning even though the
    schema keeps four weeks.
