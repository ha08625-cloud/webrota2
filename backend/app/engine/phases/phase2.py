            ...
            for day in _DAYS:
                for period in _PERIODS:
                    ...
                    if not is_within_window(doctor, date_):
                        # a doctor outside their employment window gets no
                        # slot on this date. Same "cell absence is data"
                        # mechanism as the two skips around it, so no
                        # downstream phase needs a per-slot window check.
                        # Independent of `active` (see doctor_window's
                        # docstring) -- context.doctors already applies that
                        # filter.
                        continue
...
def _load_counter_state(db: Session) -> CounterState:
    ...
    Counter adjustments travel with the counts, from the same rows. They are
    ...
    for row in db.execute(select(ClinicCounter)).scalars().all():
        ...
        counters.clinic_adjustment[key] = float(row.adjustment)
    for row in db.execute(select(SystemCounter)).scalars().all():
        ...
        counters.system_adjustment[key] = float(row.adjustment)
    return counters